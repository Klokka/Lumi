...
- (void)checkForLatestLumiAt: (NSString *)url
{
...
{
  pkgPath = [[self getTempPath] stringByAppendingPathComponent: @"lumi-setup.dmg"];
...
- (void)download: (NSURLDownload *)download didReceiveDataOfLength: (unsigned)length
{
  ...
  if (expectedLength != NSURLResponseUnknownLength) {
    ...
    [textField setStringValue: [NSString stringWithFormat: @"Downloading Lumi. (%0.1f%% done)", perc]];
    [progress setDoubleValue: perc];
  }
}
- (void)downloadDidFinish: (NSURLDownload *)download
{
  NSString *lumiMount, *lumiPath;
  ...
  lumiMount = [[self getTempPath] stringByAppendingPathComponent: @"Lumi"];
  lumiPath = [lumiMount stringByAppendingPathComponent: @"Lumi.app"]; 
  [fm copyPath: lumiPath toPath: @"/Applications/Lumi.app" handler: nil];

  mount = [[NSTask alloc] init];
  [mount setLaunchPath: @"/usr/bin/hdiutil"];
  [mount setArguments: [NSArray arrayWithObjects: @"unmount", @"-quiet", lumiMount, nil]];
  [mount launch];
  [mount waitUntilExit];
  [mount release];
  [download release];
...
#ifdef __ppc__
  [events checkForLatestLumiAt: @"http://hacketyhack.net/pkg/osx/lumi-ppc"];
#else
  [events checkForLatestLumiAt: @"http://hacketyhack.net/pkg/osx/lumi"];
#endif

  [[win contentView] addSubview: text];
  [text setStringValue: @"Downloading Lumi."];
  [text setBezeled: NO];
