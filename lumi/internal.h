//
// lumi/internal.h
// Debugging and allocation functions within Lumi.
//
...
int lumi_snprintf(char* str, size_t size, const char* format, ...);
...
#define WARN PUTS
#define QUIT(msg) \
  if (code == LUMI_OK) code = LUMI_FAIL; \
  PUTS(msg); \
  goto quit

#else

#define lumi_snprintf snprintf
