...
extern VALUE cLumi, cApp, cDialog, cTypes, cLumiWindow, cMouse, cCanvas, cFlow, cStack, cMask, cNative, cShape, cVideo, cImage, cEffect, cEvery, cTimer, cAnim, cPattern, cBorder, cBackground, cPara, cBanner, cTitle, cSubtitle, cTagline, cCaption, cInscription, cLinkText, cTextBlock, cTextClass, cSpan, cStrong, cSub, cSup, cCode, cDel, cEm, cIns, cButton, cEditLine, cEditBox, cListBox, cProgress, cSlider, cCheck, cRadio, cColor, cDownload, cResponse, cColors, cLink, cLinkHover, ssNestSlot;
...
VALUE lumi_safe_block(VALUE, VALUE, VALUE);
void lumi_ruby_init(void);
...
//
#define LUMI_META \
  "(class << Lumi; self; end).instance_eval do;"
#ifdef LUMI_WIN32
#define QUIT_ALERT_MSG() MessageBox(NULL, RSTRING_PTR(msg), "Lumi", MB_OK)
#else
...
#define DEF_COLOR(name, r, g, b) rb_hash_aset(cColors, ID2SYM(rb_intern("" # name)), lumi_color_new(r, g, b, 255))
#define GET_STRUCT(ele, var) \
  lumi_##ele *var; \
  Data_Get_Struct(self, lumi_##ele, var)
...
#define ATTR(attr, n)                  lumi_hash_get(attr, s_##n)
#define PX(attr, n, dn, pn)            lumi_px(lumi_hash_get(attr, s_##n), dn, pn, 1)
#define PXN(attr, n, dn, pn)            lumi_px(lumi_hash_get(attr, s_##n), dn, pn, 0)
#define PX2(attr, n1, n2, dn, dr, pn)  lumi_px2(attr, s_##n1, s_##n2, dn, dr, pn)
#define ATTR2(typ, attr, n, dn)        lumi_hash_##typ(attr, s_##n, dn)
#define ATTRSET(attr, k, v)            attr = lumi_hash_set(attr, s_##k, v)
#define ATTR_MARGINS(attr, dm, canvas) \
  ...
  { \
    lmargin = lumi_px(rb_ary_entry(margino, 0), dm, CPW(canvas), 1); \
    tmargin = lumi_px(rb_ary_entry(margino, 1), dm, CPH(canvas), 1); \
    rmargin = lumi_px(rb_ary_entry(margino, 2), dm, CPW(canvas), 1); \
    bmargin = lumi_px(rb_ary_entry(margino, 3), dm, CPH(canvas), 1); \
  } \
  else \
  { \
...
int lumi_px(VALUE, int, int, int);
int lumi_px2(VALUE, ID, ID, int, int, int);
VALUE lumi_hash_set(VALUE, ID, VALUE);
VALUE lumi_hash_get(VALUE, ID);
int lumi_hash_int(VALUE, ID, int);
double lumi_hash_dbl(VALUE, ID, double);
char *lumi_hash_cstr(VALUE, ID, char *);
VALUE rb_str_to_pas(VALUE);
void lumi_place_exact(lumi_place *, VALUE, int, int);
void lumi_place_decide(lumi_place *, VALUE, VALUE, int, int, unsigned char, int);
unsigned char lumi_is_element(VALUE);
unsigned char lumi_is_any(VALUE);
void lumi_extras_remove_all(lumi_canvas *);
void lumi_ele_remove_all(VALUE);
void lumi_cairo_rect(cairo_t *, double, double, double, double, double);
void lumi_cairo_arc(cairo_t *, double, double, double, double, double, double);
