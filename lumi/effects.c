...
void
lumi_gaussian_blur_filter(cairo_t *cr, VALUE attr, lumi_place *place)
{
  ...
  unsigned int dX, dY;
  dX = (unsigned int) floor(blur_x * 3*sqrt(2*LUMI_PI)/4 + 0.5);
  dY = (unsigned int) floor(blur_y * 3*sqrt(2*LUMI_PI)/4 + 0.5);
...
static void
lumi_layer_blur_filter(cairo_t *cr, VALUE attr, lumi_place *place,
...
void
lumi_shadow_filter(cairo_t *cr, VALUE attr, lumi_place *place)
{
  int distance = ATTR2(int, attr, distance, 4);
  lumi_layer_blur_filter(cr, attr, place, CAIRO_OPERATOR_IN, CAIRO_OPERATOR_DEST_OVER, distance);
}

void
lumi_glow_filter(cairo_t *cr, VALUE attr, lumi_place *place)
{
  ...
  lumi_layer_blur_filter(cr, attr, place, blur_op, merge_op, 0);
}
