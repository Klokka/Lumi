...
#
# lib/lumi.rb
# The Lumi base app, both a demonstration and the learning tool for
# using Lumi.
#

require 'lumi/encoding'
...
require 'lumi/inspect'
require 'lumi/cache'
if Object.const_defined? :Lumi
  require 'lumi/image'
end
require 'lumi/shybuilder'
...
class Lumi
  ...
  @locale = ENV["LUMI_LANG"] || ENV["LC_MESSAGES"] || ENV["LC_ALL"] || ENV["LANG"] || "C"
  @language = @locale[/^(\w{2})_/, 1] || "en"

  @mounts = []

  OPTS = OptionParser.new do |opts|
    opts.banner = "Usage: lumi [options] (app.rb or app.shy)"
    ...
    opts.on("-p", "--package",
            "Package a Lumi app for Windows, OS X and Linux.") do |s|
      make_pack
    end
  ...
  def self.splash
    ...
    Lumi.app :width => 400, :height => 300, :resizable => false do  
      ...
      stack :margin => 18 do
        ...
        para "LUMI", :size => 48, :stroke => "#DFA", :margin_top => 0
        stack do
          background black(0.2), :curve => 8
          para link("Open an App.") { Lumi.show_selector and close }, :margin => 10, :margin_bottom => 4
          #para link("Package an App.") { Lumi.package_app and close }, :margin => 10, :margin_bottom => 4
					para link("Package an App.") { Lumi.make_pack and close }, :margin => 10, :margin_bottom => 4
          para link("Read the Manual.") { Lumi.show_manual and close }, :margin => 10
        end
  ...
  def self.make_pack
    require 'lumi/pack'
    Lumi.app(:width => 500, :height => 480, :resizable => true, &PackMake)
  end
  ...
  def self.manual_as format, *args
    ...
    case format
    when :lumi
      Lumi.app(:width => 720, :height => 640, &Lumi::Help)
    else
      extend Lumi::Manual
      man = self
      dir, = args
      FileUtils.mkdir_p File.join(dir, 'static')
      FileUtils.cp "#{DIR}/static/lumi-icon.png", "#{dir}/static"
      ...
      html_bits = proc do
        ...
        when :colors
          color_names = (Lumi::COLORS.keys*"\n").split("\n").sort
          color_names.each do |color|
            c = Lumi::COLORS[color.intern]
        ...
        when :samples
          ...
          h.each do |k, v|
            ...
            v.each do |file|
              ...
              samples << "<a href=\"http://github.com/lumi/lumi/raw/master/manual-snapshots/#{k}-#{sample}.png\">#{sample}</a>"
            end
            p samples.join ' '
          end
        else
      ...
      docs = load_docs(Lumi::Manual::path)
  ...
  def self.show_log
    require 'lumi/log'
    return if @log_app and Lumi.APPS.include? @log_app
    @log_app =
      Lumi.app do
        extend Lumi::LogWindow
        setup
      end
  end
  ...
  def self.visit(path)
    ...
    when URI::HTTP
      ...
      if path =~ /\.shy$/
        ...
        tmpdir = "%s/lumi-%s.%d" % [Dir.tmpdir, base, $$]
        shy = Shy.x(path, tmpdir)
        Dir.chdir(tmpdir)
        Lumi.debug "Loaded SHY: #{shy.name} #{shy.version} by #{shy.creator}"
        path = shy.launch
      else
        @shy = false
  ...
  def self.read_file path
    if (RUBY_VERSION =~ /^1\.9/ or RUBY_VERSION =~ /^2\.0/) and !@shy
