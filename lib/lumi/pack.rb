...
class Lumi
  module Pack
    ...
    def self.pkg(platform, opt)
      ...
      case opt
      when Lumi::I_YES then
        url = "http://lumi.heroku.com/pkg/#{Lumi::RELEASE_NAME.downcase}/#{platform}/lumi"
        local_file_path = File.join(LIB_DIR, Lumi::RELEASE_NAME.downcase, platform, "latest_lumi.#{extension}")
      when Lumi::I_NOV then
        url = "http://lumi.heroku.com/pkg/#{Lumi::RELEASE_NAME.downcase}/#{platform}/lumi-novideo"
        local_file_path = File.join(LIB_DIR, Lumi::RELEASE_NAME.downcase, platform, "latest_lumi-novideo.#{extension}")
      when I_NET then
        url = false
      else
      ...
      FileUtils.makedirs File.join(LIB_DIR, Lumi::RELEASE_NAME.downcase, platform)
      ...
      if url then
        ...
        if File.exists? local_file_path
          ...
          noHopeMsg = "Failed to find an existing Lumi at:\n#{local_file_path}\nor download from\n#{url} to include with your script."
          raise noHopeMsg
        end
      end
    end
    ...
    def self.exe(script, opt, &blk)
      ...
      if f2
        ...
        Dir.chdir DIR + '/static/stubs' do
          `.\\lumi-stub-inject.exe #{script.gsub('/', "\\")}`
        end
      end
      blk[1.0] if blk
    end
    ...
    def self.dmg(script, opt, &blk)
      ...
      FileUtils.cp(File.join(DIR, "static", "Lumi.icns"), app_dir)
      FileUtils.cp(File.join(DIR, "static", "Lumi.icns"), res_dir)
...
END
      ...
      File.open(File.join(mac_dir, "#{name}-launch"), 'w') do |f|
        f << <<END
#!/bin/bash
SHOESPATH=/Applications/Lumi.app/Contents/MacOS
APPPATH="${0%/*}"
unset DYLD_LIBRARY_PATH
cd "$APPPATH"
echo "[Pango]" > /tmp/pangorc
echo "ModuleFiles=$SHOESPATH/pango.modules" >> /tmp/pangorc
if [ ! -d /Applications/Lumi.app ]
  then ./cocoa-install
    fi
    open -a /Applications/Lumi.app "#{File.basename(script)}"
    # DYLD_LIBRARY_PATH=$SHOESPATH PANGO_RC_FILE="$APPPATH/pangorc" $SHOESPATH/lumi-bin "#{File.basename(script)}"
END
      end
  ...
  Lumi::I_NET = "No, download Lumi if it's absent."
  Lumi::I_YES = "Yes, I want Lumi included."
  Lumi::I_NOV = "Yes, include Lumi, but without video support."
  PackMake = proc do
    background "#DDD"
    ...
    @page1 = stack do
      stack do
        ...
        stack :margin => 20 do
          ...
          para "Should Lumi be included with your script or should the script ",
            "download Lumi when the user runs it? Not all options are available on all ",
            "systems. The defaults work."
          flow :margin_left => 20 do
            @shy = check
            para "Lumi (.shy) for users who have Lumi already", :margin_right => 20
          end
          items = [Lumi::I_NET, Lumi::I_YES, Lumi::I_NOV]
          ...
          flow :margin_left => 20 do
            ...
            osxop = [Lumi::I_NET, Lumi::I_NOV]
            @incOSX = list_box :items => osxop, :width => 0.6, :height => 30 do
              @downOpt = @incOSX.text
              est_recount
            end
            @incOSX.choose(Lumi::I_NOV)
          end
          flow :margin_left => 20 do
            flow :width => 0.25 do
              @run = check
            ...
            @incLinux = list_box :items => [Lumi::I_NET], :width => 0.6,
            :height => 30 do
              est_recount
            end
            @incLinux.choose(Lumi::I_NET)
          end
        end
      end
      ...
      stack :margin => 20 do
        ...
        def est_recount
          ...
          case  @downOpt
          when Lumi::I_NET; 98
          when Lumi::I_YES; 11600
          when Lumi::I_NOV; 7000
          end
        ...
        inscription "Using the latest Lumi build (0.r#{Lumi::REVISION})", :margin => 0
    ...
    start do
      ...
      @run.checked = false
      @shy.checked = true
      #@inc.choose( ::RUBY_PLATFORM =~ /mswin|mingw/ ? Lumi::I_NET : Lumi::I_NOV )
    end
  end
end
