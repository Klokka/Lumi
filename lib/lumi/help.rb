...
module Lumi::Manual
  ...
  [INTRO_STYLE, SUB_STYLE].each do |h|
    h[:font] = "MS UI Gothic"
  end if Lumi.language == 'ja'

  def self.path
    path = "#{DIR}/static/manual-#{Lumi.language}.txt"
  ...
  def load_docs path
    return @docs if @docs
    str = Lumi.read_file(path)
    @search = Lumi::Search.new
            ...
            found.each do |typ, head|
              flow :margin => 4 do
                ...
                when "S"
                  ...
                  para "Section header", Lumi::Manual::SUB_STYLE
                when "T"
                  background "#777", :curve => 4
                  caption strong(link(head, :stroke => "#EEE") { open_methods(head, terms.text) })
                  hsh = @methods[head]
                  para "Sub-section under #{hsh['section']} (#{hsh['methods'].length} methods)", Lumi::Manual::SUB_STYLE
                when "M"
                  background "#CCC", :curve => 4
                  sect, subhead, head = head.split(Lumi::Manual::COLON, 3)
                  para strong(sect, Lumi::Manual::COLON, subhead, Lumi::Manual::COLON, link(head) { open_methods(subhead, terms.text, head) })
                end
              end
            end
          end
...
def Lumi.make_help_page
  font "#{DIR}/fonts/Coolvetica.ttf" unless Lumi::FONTS.include? "Coolvetica"
  proc do
    extend Lumi::Manual
    docs = load_docs Lumi::Manual.path

    style(Lumi::Image, :margin => 8, :margin_left => 100)
    style(Lumi::Code, :stroke => "#C30")
    style(Lumi::LinkHover, :stroke => green, :fill => nil)
    style(Lumi::Para, :size => 12, :stroke => "#332")
    style(Lumi::Tagline, :size => 12, :weight => "bold", :stroke => "#eee", :margin => 6)
    style(Lumi::Caption, :size => 24)
    background "#ddd".."#fff", :angle => 90
    
    [Lumi::LinkHover, Lumi::Para, Lumi::Tagline, Lumi::Caption].each do |type|
    ...
    stack do
      ...
      stack :margin_left => 118 do
        para "The Lumi Manual", :stroke => "#eee", :margin_top => 8, :margin_left => 17, 
          :margin_bottom => 0
    ...
    add_next_link(0, -1)
    stack :top => 80, :left => 0, :attach => Lumi::Window do
      @toc = {}
      ...
      stack :margin => 12, :width => 118 do
        inscription "Lumi #{Lumi::RELEASE_NAME}\nRevision: #{Lumi::REVISION}",
          :size => 7, :align => "center", :stroke => "#999"
      end
    end
    image :width => 120, :height => 120, :top => -18, :left => 6 do
      image "#{DIR}/static/lumi-icon.png", :width => 100, :height => 100, :top => 10, :left => 10 
      glow 2
    end
  end
rescue => e
